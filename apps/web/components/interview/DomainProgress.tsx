...
export function DomainProgress({ onRevisit }: { onRevisit?: (id: string) => void }) {
  ...
  const remaining = activeDomains.length - completedDomains.length;
  const estMin = Math.max(1, Math.round(remaining * 1.5));
  return (
    <aside className="w-full md:w-[260px] flex-shrink-0 md:border-r border-hairline bg-sidebar-mist flex flex-col overflow-hidden">
      ...
      <div role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} className="h-1 bg-edge-gray rounded-full mx-4 mb-2.5"><div className="h-full bg-ink rounded-full transition-all" style={{width:`${pct}%`}}/></div>
      <div className="flex-1 overflow-y-auto">
        {activeDomains.map((d)=>{
          const done=completedDomains.includes(d.id), active=d.id===currentDomain&&!isComplete;
          const canGoBack = done && !busy;
          const est = d.mode === "open" ? "~2m" : "~1m";
          const row = (
            <>
              ...
              {!done && !active && <span className="ml-auto text-[10px] text-ink-faint">{est}</span>}
            </>
          );
          return canGoBack ? (
      ...
      <div className="px-4 py-2.5 border-t border-hairline text-[12px] text-ink-muted">{completedDomains.length} / {activeDomains.length} domains{remaining > 0 ? ` · ~${estMin} min left` : " · done"}</div>
    </aside>
  );
}
