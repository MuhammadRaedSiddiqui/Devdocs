...
import { SchemaTable } from "@/components/interview/SchemaTable";

function mdLite(t: string) {
  const html = t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\n/g,"<br/>");
  return <span dangerouslySetInnerHTML={{__html:html}}/>;
}
function Avatar({role}:{role:"user"|"assistant"}) {
  return <div className={`w-[26px] h-[26px] rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-medium ${role==="user"?"bg-[#ccdbe8] text-[#0c447c]":"bg-ink text-vellum"}`}>{role==="user"?"U":"AI"}</div>;
}
function Dots() {
...
function Bubble({message,context,onConfirmCard}:{message:ChatMessage;context:ProjectContext;onConfirmCard:(id:string)=>void}) {
  ...
  return (
    <div className={`flex gap-2.5 items-start ${isUser?"flex-row-reverse":""}`}>
      ...
      <div className={`max-w-[80%] px-3.5 py-2.5 rounded-vellum text-sm leading-relaxed border ${isUser?"bg-vellum text-ink border-vellum-border":message.isComplete?"bg-[#fff8f5] text-ink-secondary border-terracotta":"bg-white text-ink-secondary border-vellum-border"}`}>
        {mdLite(message.content)}
        ...
        {message.showSchema && <SchemaTable tables={store.schemaTables} interactive={!store.schemaConfirmed} onAddField={store.addSchemaField} onHelp={()=>store.sendMessage("I need help understanding this")} onConfirm={()=>store.confirmSchema()}/>}
        {message.showDownload && <button type="button" onClick={handleDownload} className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 bg-ink text-vellum rounded-vellum text-xs font-medium">↓ Download DOCUMENTATION.md</button>}
      </div>
    </div>
  );
}
...
export function ChatPanel() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden min-w-0">
      {/* Context bar */}
      <div className="px-5 py-2 text-xs text-ink-muted border-b border-vellum-border bg-vellum flex items-center gap-1.5 flex-wrap flex-shrink-0">
        <span className="text-ink-faint">Context:</span>
        <span className="bg-ink text-vellum border border-ink rounded-md px-2 py-0.5 text-[11px]">{PROJECT_TYPE_LABELS[ctx.projectType]}</span>
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
        {store.messages.map((m,i)=><Bubble key={i} message={m} context={ctx} onConfirmCard={handleConfirmCard}/>)}
        {store.isThinking&&<div className="flex gap-2.5 items-start"><Avatar role="assistant"/><div className="bg-white border border-vellum-border rounded-vellum px-3.5 py-3"><Dots/></div></div>}
      ...
      <div className="border-t border-vellum-border px-5 py-3.5 flex gap-2 items-end flex-shrink-0 bg-white">
        <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if((e.metaKey||e.ctrlKey)&&e.key==="Enter")handleSend();}} disabled={disabled} rows={1} placeholder={disabled?"AI is responding...":store.isComplete?"Ask a question or request a change...":"Type your answer..."} className="flex-1 px-3.5 py-2.5 border border-ink/15 rounded-vellum text-[13px] bg-white text-ink resize-none min-h-[42px] max-h-[88px] disabled:opacity-45 focus:outline-none focus:border-ink/30"/>
        <button type="button" onClick={handleSend} disabled={disabled} className="px-4 py-2.5 bg-ink text-vellum rounded-vellum text-[13px] font-medium disabled:opacity-35">Send</button>
      </div>
    </div>
  );
}
