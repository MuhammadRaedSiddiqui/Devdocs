    ...
    <div className="max-w-[640px]">
      ...
      <div className="bg-white border border-vellum-border rounded-vellum mb-4 p-4 flex items-center justify-between">
        <div className="flex-1 mr-4">
          ...
          <div className="h-1.5 bg-edge-gray rounded-full"><div className="h-full bg-ink rounded-full" style={{width:`${pct}%`}}/></div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={p.tier} className={`bg-white border rounded-vellum p-[18px] relative ${p.current?"border-ink border-[1.5px]":"border-vellum-border"}`}>
            <span className={`absolute -top-[9px] left-4 text-[10px] px-2.5 py-0.5 rounded-none font-medium ${p.current?"bg-ink text-white":"badge-amber"}`}>{p.badge}</span>
